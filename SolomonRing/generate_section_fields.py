...
_TARGET_SCOPE_HELP = (
    ...
    "        'Everyone on one side' name was wrong. BUT IT DOES NOT WORK: see below.\n"
    ...
    "0x20 works for an ENEMY attack but NOT for a player command. At damage time "
    "processMultiHitAttackExecution routes any mask carrying 0x8000 through "
    "expandTargetMaskToValidSide (0x48EE50), which throws away the slot bits and re-derives "
    "one side: == 0xFF -> everyone, <= 0x07 -> the party, anything else -> the monsters. "
    "That 0xFF needs bit 7, and bit 7 is a PHANTOM SLOT: the engine has only 7 battle slots "
    "(0-2 party, 3-6 monsters - four monsters maximum), so both live-slot mask builders "
    "produce at most 0x7F.\n"
    "  Enemy / auto-resolved actions: getMaskEveryone returns the hardcoded constant 0x00FF "
    "| 0x8000 - phantom bit included - so it matches == 0xFF exactly and everyone is hit.\n"
    "  Player commands: sub_4AA1D0 builds the mask from the live slot set, which can never "
    "be 0xFF, so it always falls through to monsters-only. The cursor still highlights "
    "everyone (the menu draws from the unexpanded mask), so the selection looks right and "
    "only the damage is one-sided. No data combination fixes this - not even a maximally "
    "full battlefield, since bit 7 is unreachable; it needs an exe patch at 0x48EE50."
)
...
_DUEL_BUTTON_HELP = (
    ...
    "(stored & 0xF0FF). Bits 0-7 come from the engine pad mask (read_pad_pressed_raw, truncated "
    "to its low byte by the caller), bits 12-15 from the D-pad. In vanilla every slot holds exactly "
    ...
    "WHAT THE PLAYER ACTUALLY PRESSES: each bit is a remappable SLOT, not a fixed key, so the "
    "names give the slot's default gamepad button plus the ff8input.cfg command that rebinds it. "
    "Create_ff8input_cfg (0x498CB0) writes the 14 commands in order and sub_498550 maps each to "
    "its pad bit: 1 Select -> 0x20 (Circle), 2 Exit -> 0x40 (Cross), 3 Misc -> 0x80 (Square), "
    "4 Menu -> 0x10 (Triangle), 5 Toggle -> 0x04 (L1), 6 Trigger -> 0x08 (R1), 7 RotLt -> 0x01 "
    "(L2), 8 RotRt -> 0x02 (R2), 9 Start -> 0x800, 10 Select -> 0x100, and 11-14 Up/Down/Left/"
    "Right -> 0x1000/0x4000/0x8000/0x2000. Rebinding a command changes which key performs that "
    "input; the value stored here does not change.")
_DUEL_FINISHER_HELP = (
