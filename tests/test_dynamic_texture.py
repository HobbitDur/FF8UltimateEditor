from FF8GameData.monsterdata import DynamicTextureSection
from Ifrit.ifritmanager import IfritManager
from IfritDynamicTexture.ifritdynamictexturewidget import IfritDynamicTextureWidget


class TestDynamicTexture:

    def test_save_pupu_no_change(self):
        data_origin = bytearray([
            0x08, 0x00, 0x1C, 0x00, 0x28, 0x00, 0x38, 0x00,
            0x01, 0x08, 0x34, 0x15, 0x0D, 0x05, 0x08, 0x00,
            0x25, 0x1E, 0x25, 0x1E, 0x15, 0x59, 0x15, 0x59,
            0x00, 0x59, 0x00, 0x59, 0x01, 0x08, 0x34, 0x15,
            0x0D, 0x01, 0x00, 0x00, 0x25, 0x1E, 0x00, 0x73,
            0x01, 0x08, 0x34, 0x15, 0x0D, 0x03, 0x08, 0x00,
            0x15, 0x66, 0x15, 0x66, 0x15, 0x73, 0x15, 0x73,
            0x01, 0x33, 0x2C, 0x09, 0x1E, 0x0B, 0x08, 0x00,
            0x37, 0x00, 0x2E, 0x00, 0x25, 0x00, 0x2A, 0x2C,
            0x2A, 0x2C, 0x2A, 0x4A, 0x2A, 0x4A, 0x2A, 0x2C,
            0x2A, 0x2C, 0x25, 0x00, 0x2E, 0x00, 0x37, 0x00
        ])
        dynamic_texture_data = DynamicTextureSection()
        dynamic_texture_data.analyze(data_origin)
        data_result = dynamic_texture_data.to_binary()
        print(f"Original data: {data_origin.hex(sep=" ")}")
        print(f"Result data  : {data_result.hex(sep=" ")}")
        assert bytes(data_origin) == bytes(data_result)

    def test_save_pupu_add_entry(self):
        data_origin = bytearray([
            0x08, 0x00, 0x1C, 0x00, 0x28, 0x00, 0x38, 0x00,
            0x01, 0x08, 0x34, 0x15, 0x0D, 0x05, 0x08, 0x00,
            0x25, 0x1E, 0x25, 0x1E, 0x15, 0x59, 0x15, 0x59,
            0x00, 0x59, 0x00, 0x59, 0x01, 0x08, 0x34, 0x15,
            0x0D, 0x01, 0x00, 0x00, 0x25, 0x1E, 0x00, 0x73,
            0x01, 0x08, 0x34, 0x15, 0x0D, 0x03, 0x08, 0x00,
            0x15, 0x66, 0x15, 0x66, 0x15, 0x73, 0x15, 0x73,
            0x01, 0x33, 0x2C, 0x09, 0x1E, 0x0B, 0x08, 0x00,
            0x37, 0x00, 0x2E, 0x00, 0x25, 0x00, 0x2A, 0x2C,
            0x2A, 0x2C, 0x2A, 0x4A, 0x2A, 0x4A, 0x2A, 0x2C,
            0x2A, 0x2C, 0x25, 0x00, 0x2E, 0x00, 0x37, 0x00
        ])
        ifrit_manager = IfritManager()
        dynamic_section_widget = IfritDynamicTextureWidget(ifrit_manager)

        ifrit_manager.enemy.dynamic_texture_data.analyze(data_origin)
        dynamic_section_widget.dynamic_texture_widget.entry_widget.
        data_result = dynamic_texture_data.to_binary()
        print(f"Original data: {data_origin.hex(sep=" ")}")
        print(f"Result data  : {data_result.hex(sep=" ")}")
        assert bytes(data_origin) == bytes(data_result)
