...
class AICodeGenerator:
    ...
    def _calculate_remaining_size(self, current_elif, remaining_elifs, else_block):
        ...
        for i in range(len(remaining_elifs)):
            if current_elif == remaining_elifs[i]:
                current_elif_index = i
        ...
        for i in range(current_elif_index+1, len(remaining_elifs)):
                total_size += 3  # JUMP command
                total_size += 8  # IF command
                total_size += self._calculate_block_size(remaining_elifs[i].block)
