"""
ShumiTranslator CLI Tool.

Provides command-line interface for ShumiTranslator operations:
  • load + save binary
  • export CSV  (load → csv)
...
def _apply_csv_to_sections(csv_path: str, delimiter: str, sections: List[Tuple]):
...
def _cmd_export_all_field(args):
...
def _cmd_export_all_battle(args):
...
def _cmd_export_all_kernel(args):
...
def _cmd_export_all_namedic(args):
...
def _cmd_export_all_mngrp(args):
...
def _cmd_export_all_world(args):
...
# ---------------------------------------------------------------------------
# ShumiTranslatorCliTool Class
# ---------------------------------------------------------------------------

class ShumiTranslatorCliTool(BaseCliTool):
    """CLI tool for ShumiTranslator operations."""

    @property
    def name(self) -> str:
        """Unique tool identifier."""
        return "shumi-translator"

    @property
    def description(self) -> str:
        """Human-readable description."""
        return "ShumiTranslator - Translate FF8 game text (kernel, battle, field, etc.)"

    def build_parser(self) -> argparse.ArgumentParser:
        """Build tool-specific argument parser."""
        parser = argparse.ArgumentParser(
            prog="ff8-cli shumi-translator",
        ...
        p_export_field.set_defaults(func=_cmd_export_all_field)
        ...
        p_export_battle.set_defaults(func=_cmd_export_all_battle)
        ...
        p_export_kernel.set_defaults(func=_cmd_export_all_kernel)
        ...
        p_export_namedic.set_defaults(func=_cmd_export_all_namedic)
        ...
        p_export_mngrp.set_defaults(func=_cmd_export_all_mngrp)
        ...
        p_export_exe.set_defaults(func=_cmd_export_all_exe)
        ...
        p_export_world.set_defaults(func=_cmd_export_all_world)
        ...
        p_uncompress.set_defaults(func=_cmd_uncompress)
