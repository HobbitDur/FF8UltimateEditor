"""Trepies: editor logic for the tutorial demo data stored in mngrp.bin.
...
class TrepiesManager:
    """Loads mngrp.bin + mngrphd.bin and edits the tutorial demo raw slots."""
    ...
    def __init__(self, game_data: GameData):
        ...
        self.file_mngrphd = ""
        self._mngrp_data = b""
        self._mngrphd_data = b""
        self._entries = {}  # raw slot -> (offset, size) for valid entries
    ...
    def load_file(self, file_mngrphd: str, file_mngrp: str):
        with open(file_mngrphd, "rb") as in_file:
            self._mngrphd_data = in_file.read()
        with open(file_mngrp, "rb") as in_file:
            self._mngrp_data = in_file.read()
        self.file_mngrphd = file_mngrphd
        self.file_mngrp = file_mngrp

        self._entries = {}
        for raw_slot in range(min(HEADER_ENTRY_COUNT, len(self._mngrphd_data) // HEADER_ENTRY_SIZE)):
            seek, size = struct.unpack_from("<II", self._mngrphd_data, raw_slot * HEADER_ENTRY_SIZE)
            if size == 0 or seek in (0, 0xFFFFFFFF):
                continue
            # Bit 0 of the seek is the "stored uncompressed" flag (always set
            # on PC), hence the -1 to get the real offset.
            self._entries[raw_slot] = (seek - 1, size)

        needed = set(SCRIPT_SLOTS) | set(MOCK_CHAR_SLOTS) | set(MOCK_GF_SLOTS)
        missing = sorted(needed - set(self._entries))
        if missing:
            raise ValueError(f"mngrphd.bin misses tutorial demo raw slots {missing} - not a vanilla-layout mngrp?")
        ...
        for caption_slot in CAPTION_SLOTS:
            if caption_slot in self._entries:
                self._captions[caption_slot] = self._parse_caption_section(caption_slot)

    def _raw_section(self, raw_slot: int) -> bytes:
        offset, size = self._entries[raw_slot]
        return self._mngrp_data[offset:offset + size]
    ...
    def save_file(self, file_mngrp: str = "", file_mngrphd: str = ""):
        file_mngrp = file_mngrp or self.file_mngrp
        file_mngrphd = file_mngrphd or self.file_mngrphd
        new_sections = self._new_section_bytes()

        if all(len(new_sections[slot]) == self._entries[slot][1] for slot in new_sections):
            # No size change: splice the edited sections in place, the header
            # and every other byte of mngrp.bin stay untouched.
            mngrp = bytearray(self._mngrp_data)
            for slot, data in new_sections.items():
                offset, size = self._entries[slot]
                mngrp[offset:offset + size] = data
            mngrphd = self._mngrphd_data
        else:
            # A script grew past its sector padding: re-layout every valid
            # section contiguously (original offset order) and rebuild the
            # valid header entries; invalid entries are kept byte-exactly.
            mngrp = bytearray()
            mngrphd = bytearray(self._mngrphd_data)
            new_entries = {}
            for slot in sorted(self._entries, key=lambda raw_slot: self._entries[raw_slot][0]):
                data = new_sections.get(slot)
                if data is None:
                    data = self._raw_section(slot)
                new_entries[slot] = (len(mngrp), len(data))
                mngrp.extend(data)
            for slot, (offset, size) in new_entries.items():
                # +1 = the "stored uncompressed" seek flag (bit 0).
                struct.pack_into("<II", mngrphd, slot * HEADER_ENTRY_SIZE, offset + 1, size)

        with open(file_mngrp, "wb") as out_file:
            out_file.write(mngrp)
        with open(file_mngrphd, "wb") as out_file:
            out_file.write(mngrphd)
        # Keep the in-memory originals in sync so further edits/saves diff
        # against what is now on disk.
        self._mngrp_data = bytes(mngrp)
        self._mngrphd_data = bytes(mngrphd)
        self._entries = {}
        for raw_slot in range(min(HEADER_ENTRY_COUNT, len(self._mngrphd_data) // HEADER_ENTRY_SIZE)):
            seek, size = struct.unpack_from("<II", self._mngrphd_data, raw_slot * HEADER_ENTRY_SIZE)
            if size == 0 or seek in (0, 0xFFFFFFFF):
