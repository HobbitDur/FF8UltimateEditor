...
class XlsxToDat:
    ...
    def read_text(game_data: GameData, sheet, enemy: MonsterAnalyser):
        combat_text_list = []
        text_col = COL_MONSTER_INFO + 2

        text_range = sheet.iter_rows(min_row=ROW_MONSTER_COMBAT_TEXT + 1,
                                     max_row=ROW_MONSTER_COMBAT_TEXT + MAX_COMBAT_TXT,
                                     min_col=text_col, max_col=text_col,
                                     values_only=True)
    ...
    def read_ability(game_data: GameData, sheet, enemy: MonsterAnalyser):
        ...
        for abilities in AIData.ABILITIES_HIGHNESS_ORDER:
            ...
            for row in block_range:
                ...
                ability_type = int(type_str.split(':')[0]) if type_str and ':' in type_str else 0
                ability_id = int(id_str.split(':')[0]) if id_str and ':' in id_str else 0
    ...
    def read_stat(game_data: GameData, sheet, enemy: MonsterAnalyser):
        ...
        for row_idx, row in enumerate(stat_range):
            if row_idx < len(stat_names):
                enemy.info_stat_data[stat_names[row_idx]] = list(row)
    ...
    def read_def(game_data: GameData, sheet, enemy: MonsterAnalyser):
        ...
        # Read all defense values in one batch
    ...
    def read_monster_name(game_data: GameData, sheet, enemy: MonsterAnalyser):
        name_col = COL_MONSTER_INFO + 2
        name_cell = sheet.cell(ROW_MONSTER_NAME + 1, name_col).value

        enemy.info_stat_data['monster_name'] = FF8Text(game_data=game_data, own_offset=0, data_hex=bytearray(), id=0)
        enemy.info_stat_data['monster_name'].set_str(name_cell)

    @staticmethod
    def read_original_file(sheet):
        name_col = COL_MONSTER_INFO + 2
        return sheet.cell(ROW_ORIGINAl_FILE_NAME, name_col).value