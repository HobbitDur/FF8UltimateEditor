"""The SeeD written test data of mngrp.bin, read from a shared MngrpManager.

No Qt here: the Shiva tab and the CLI both read the SeeD tests this way, so they cannot
disagree on the section layout or the answer-byte handling.

The tests live in the mngrp string sections at raw header entries 95-126 (positions 42-73
among the valid sections): entry 95 is the shared exam UI text, entries 96-125 the tests 1-30
(10 questions each, the engine loads entry 96 + test number), entry 126 an unused "test 31"
duplicate of test 30. Each string is {u8 expected answer, FF8 text}, unlike a plain mngrp
string, which is why this is read here and not as a generic SectionString.
"""
import re
...
class SeedTest:
    ...
    def __init__(self, name, section_pos, strings):
        self.name = name
        self.section_pos = section_pos  # Section id inside mngrp (== position among the valid sections)
...
class SeedTestSet:
    """The SeeD test sections of a shared mngrp: the exam UI section and the 31 tests.

    Reads and writes only those sections of the shared model, so the rest of mngrp.bin belongs
    to the other tabs. The exam text lives in section 42, the tests in sections 43-73 (the id is
    the same as the position among the valid sections, so get_section_by_id reaches them)."""
    ...
    GENERAL_TEST_ID = "general"  # CSV/CLI row key for the shared exam UI section
    CSV_HEADER = ["Test", "String", "Answer", "Text"]

    def __init__(self, game_data: GameData, general_section, test_list):
        self.game_data = game_data
        self.general_section = general_section  # SeedTest with the shared exam UI strings
        self.test_list = test_list  # [SeedTest], index 0 = test 1

    @classmethod
    def from_mngrp(cls, game_data: GameData, mngrp):
        general_section = SeedTest.from_section_bytes(
            game_data, mngrp.get_section_by_id(cls.GENERAL_SECTION_POS).get_data_hex(),
            name="Exam UI text", section_pos=cls.GENERAL_SECTION_POS)
        test_list = []
        for test_index in range(cls.NB_TESTS):
            section_pos = cls.FIRST_TEST_POS + test_index
            name = f"Test {test_index + 1}" if test_index < 30 else "Test 31 (unused)"
            test_list.append(SeedTest.from_section_bytes(
                game_data, mngrp.get_section_by_id(section_pos).get_data_hex(),
                name=name, section_pos=section_pos))
        return cls(game_data, general_section, test_list)

    def save_to_mngrp(self, mngrp):
        """Write each SeeD section back into the shared mngrp. The file itself is written by
        MngrpManager, so nothing else of mngrp.bin is touched."""
    ...
    def to_csv_rows(self):
        """One row [test id, string index, answer, decoded text] per string (CSV export)."""
        rows = []
