...
from PyQt6.QtWidgets import (QWidget, QPushButton, QVBoxLayout, QHBoxLayout, QListWidget, QSpinBox,
                             QComboBox, QGroupBox, QFormLayout, QTabWidget, QTableWidget,
                             QTableWidgetItem, QHeaderView, QLineEdit, QCheckBox, QScrollArea,
                             QMessageBox)

from Shiva.ShivaTutorial.tutorialmanager import (TutorialManager, DemoScriptOp, DEMO_OPCODE_NAMES,
...
class ShivaTutorialWidget(QWidget):
    """mngrp.bin tutorial demo editor (was the Trepies tool), a tab of Shiva.
    ...
    It edits only those sections of the shared mngrp; Shiva writes the file."""

    def __init__(self, game_data):
        QWidget.__init__(self)

        self.game_data = game_data
        if not self.game_data.magic_data_json:
            self.game_data.load_magic_data()
        self.manager = None  # TutorialManager, built from the shared mngrp on load
        self._loaded = False
    ...
    def owned_section_ids(self, manager):
        ...
        return self.manager.owned_section_ids() if self.manager else set()

    def load_from_mngrp(self, manager):
        self.manager = TutorialManager.from_mngrp(self.game_data, manager.mngrp)
        self._loaded = True
    ...
    def save_to_mngrp(self, manager):
        if self.manager:
            self.manager.save_to_mngrp(manager.mngrp)
    ...
    def _on_gf_changed(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Shiva - Tutorial demos", str(e))
            with QSignalBlocker(self.gf_name_edit):
                self.gf_name_edit.setText(record.name)
        for key, spinbox in self.gf_fields.items():
            setattr(record, key, spinbox.value())
