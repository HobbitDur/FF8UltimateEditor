"""Render a mmag.bin / mmag2.bin page the way the menu draws it, into a PIL image.

The magazine viewers fill a PSX ordering table, so the primitives added last are
drawn first. Back to front a magazine page is:

    paper background -> menu window -> paper mat -> picture overlays
    -> unlock block -> text -> footer

Which magazine viewer draws it depends on the entry: the tutorial books (43-67)
go through Menu_Magazine_Draw (0x4C9330), the item-menu magazines (0-42) through
Menu_ItemMagazine_DrawPage (0x4FD6E0). They agree on everything above except the
unlock block, which only the item reader draws (and drawing it is what unlocks
the weapon/move in the savemap, every frame the page is up).

The Chocobo World screen (mmag2.bin, Menu_ChocoboWorld_Draw at 0x4D1D30) reuses
the same 68-byte entry but draws **only the two overlay layers** — no window,
paper, mat, unlock block or footer; those belong to the screen's own chrome,
which is not reconstructible from mmag2.bin. The manager says which layers apply
through its DRAWS_* flags, and render() uses them as the per-layer default.
...
class PageRenderer:
    """Draws mmag / mmag2 page entries. Needs a MagPageManager with mngrp loaded
    for the page textures, sprites and text, and the menu folder for sysfnt.*

    Each layer defaults to the manager's DRAWS_* flag (the magazine draws them
    all; the Chocobo World screen only the overlays); pass an explicit True/False
    to a render() argument to override one."""
    ...
    def render(self, entry: MagPageEntry, draw_background=None, draw_mat=None,
               draw_pictures=None, draw_unlock=None, draw_text=None, draw_footer=None):
        canvas = Image.new("RGBA", (CANVAS_WIDTH, CANVAS_HEIGHT), BACKDROP_COLOR)
        if self._flag(draw_background, "DRAWS_BACKGROUND"):
            self._draw_background(canvas, entry)
        if self._flag(draw_mat, "DRAWS_MAT"):
            self._draw_mat(canvas, entry)
        if self._flag(draw_pictures, "DRAWS_PICTURES"):
            self._draw_picture_overlays(canvas, entry)
        if self._flag(draw_unlock, "DRAWS_UNLOCK"):
            self._draw_unlock(canvas, entry)
        if self._flag(draw_text, "DRAWS_TEXT"):
            self._draw_text_overlays(canvas, entry)
        if self._flag(draw_footer, "DRAWS_FOOTER") and entry.footer_flag:
