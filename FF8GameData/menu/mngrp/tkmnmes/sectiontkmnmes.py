...
class SectionTkmnmes(Section):
    ...
    def get_text_section_by_slot(self, slot):
        """The subsection at the given offset slot, or None when the slot is empty.

        getMenuString (0x4BD630) reads subtable = file[offset_table[section]], so the section
        number is an offset slot: an empty slot returns EMPTY_STRING. The subsections built
        here skip the empty slots, so their position is not their slot number."""
        for section_text in self._string_section_list:
            if section_text.id == slot:
                return section_text
        return None
