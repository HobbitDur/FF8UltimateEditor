...
class SectionString(Section):
    ...
    def get_text_by_slot(self):
        """The strings indexed by offset slot, the way the engine reads them.

        Menu_Magazine_Draw does word_1D773A6[text_id] (0x4C9436): the text id is a raw index
        in the offset table, so a slot with a zero offset holds no string but still takes its
        place. get_text_list() skips those slots, which is what a translator wants to see but
        shifts every id after them."""
        text_list = self._text_section.get_text_list()
        slot_text_list = []
        index_text = 0
        for offset in self._offset_section.get_all_offset():
            if offset == 0:
                slot_text_list.append("")
            else:
                slot_text_list.append(str(text_list[index_text]))
                index_text += 1
        return slot_text_list
